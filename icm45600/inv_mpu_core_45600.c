			...
			struct device_attribute *attr, char *buf)
{
	struct iio_dev *indio_dev = dev_get_drvdata(dev);
	struct inv_mpu_state *st = iio_priv(indio_dev);
	int res, ret, accel_fs, gyro_fs;

	mutex_lock(&st->lock);
	accel_fs = st->chip_config.accel_fs;
	gyro_fs = st->chip_config.fsr;
	res = inv_hw_self_test(st);
	inv_mpu_initialize(st);
	st->chip_config.accel_fs = accel_fs;
	st->chip_config.fsr = gyro_fs;
	ret = inv_set_accel_sf(st);
	if (ret)
		pr_err("inv_set_accel_sf error %d\n", ret);
	ret = inv_set_gyro_sf(st);
	if (ret)
		pr_err("inv_set_gyro_sf error %d\n", ret);
	mutex_unlock(&st->lock);
			...
			struct device_attribute *attr, char *buf)
{
	...
	inv_plat_read(st, REG_TEMP_DATA1_UI_DREG_BANK1, 2, (u8 *)&d);
	temp = (s16)be16_to_cpu(d);

	if ((v & 0x0f) == 0) {
		inv_plat_single_write(st, REG_PWR_MGMT0_DREG_BANK1, v);
		usleep_range(5000, 7000);
	}
	/* output temperature is scaled by 100 */
	out_temp = temp * 100;
	out_temp = (out_temp >> 7);
	out_temp += 2500;

	return snprintf(buf, MAX_WR_SZ, "%d %lld\n", out_temp, get_time_ns());
}
...
static IIO_DEVICE_ATTR(in_calib_magn_enable, S_IRUGO | S_IWUSR, inv_sensor_on_show,
			inv_sensor_on_store, SENSOR_L_MAG_CAL);
...
static IIO_DEVICE_ATTR(in_calib_magn_rate, S_IRUGO | S_IWUSR, inv_sensor_rate_show,
			inv_sensor_rate_store, SENSOR_L_MAG_CAL);
